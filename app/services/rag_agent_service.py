...
class RagAgentService:
    ...
    async def query(
        ...
        try:
            ...
            # 构建消息列表（系统提示 + 摘要 + 历史 + 当前问题）
            prompt_parts = [self.system_prompt]
            if summary:
                prompt_parts.append(f"\n[历史摘要]: {summary}")
            if history:
                # 取最近几轮历史（排除当前用户问题）
                recent = [m for m in history if m["content"] != question][-6:]
                if recent:
                    prompt_parts.append("\n[最近对话]:")
                    for m in recent:
    ...
    async def query_stream(
        ...
        try:
            ...
            # 流完成后写入完整回答到 PostgreSQL
            full_answer = "".join(full_answer_parts)
            if full_answer:
                await memory_service.add_assistant_message(session_id, full_answer)
                await memory_service.check_and_summarize(session_id)
    ...
    async def get_session_history(self, session_id: str) -> list:
        ...
        """
        try:
            history = await memory_service.get_history(session_id, limit=50)
