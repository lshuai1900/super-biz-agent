...
基于 Ragas 的自动质量评估系统。需要配置 DashScope API Key。
...
```bash
# 快速模式：生成 3 条测试问题（约 10-30 秒）
curl -X POST http://localhost:9900/api/evaluation/generate_dataset \
  -H "Content-Type: application/json" \
  -d '{"source_dir":"aiops-docs","quick":true,"count":3}'
...
| `count` | 3 | 生成数量（最大 20） |
| `max_docs` | 3 | 最多读取文档数 |
| `max_chunks` | 20 | 最多使用 chunk 数 |
| `timeout_seconds` | 120 | 超时时间（秒） |
| `use_cache` | true | 是否使用缓存 |
| `force` | false | 强制重新生成（跳过缓存） |
| `quick` | false | 快速演示模式（自动限制参数规模） |

### 生成慢怎么办

- 使用 `quick=true` 快速模式（count=3, max_docs=2, timeout=60s）
- 减少 `count`、`max_docs` 或 `max_chunks`
- 增大 `timeout_seconds`
- 生成结果会自动缓存到 `data/evaluation/test_dataset.json`，再次请求直接返回缓存
- 使用 `force=true` 可跳过缓存重新生成

### 清除缓存

```bash
# 删除缓存文件即可
rm -f data/evaluation/test_dataset.json
```
